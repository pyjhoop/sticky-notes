 ...
 *
 * 디자인 버전1(114~119행)의 동기화 푸터를 **레이아웃 그대로 두고 의미만** 바꾼 것이다
 * (`plan.md` "디자인 대비 변경점"). 옵시디언 동기화는 v1 범위 밖이므로
 * `Vault` · `동기화` 같은 문구는 쓰지 않는다.
 *
 * 2026-08-03 v2 2단계(A트랙) — design v2(StickyNote App.dc.html 48~49행)의
 * `수정 2분 전 / 자동 저장됨` 모노 한 줄이 이 자리에 해당한다. 로직(아래 상태 점 ·
 * "저장됨 · HH:mm")은 그대로 두고, 시각만 그 모노/저채도 스타일로 맞췄다(note.css).
 *
