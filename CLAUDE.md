...
코드블록 (배경/본문/키워드/함수/타입/주석은 design v2 실측, string/number/operator/variable
은 2단계 A트랙이 같은 hue 배치·대비 규칙으로 새로 확정 — 판단 근거는 tokens.css 주석)
  ...
  문자열 oklch(0.8 0.12 104)   숫자 oklch(0.78 0.13 32)
  연산자 oklch(0.7 0.03 68)    변수 oklch(0.75 0.13 356)

다크 크롬 (관리 창 · 설정 창 · 트레이 메뉴) — **2026-08-03 라이트로 전환 완료** (design v2가
이 세 창을 전부 밝은 배경으로 그렸고 "코드블록만 다크 서피스"라고 명시한 데 따른 결정).
클래스명(`.dark-*`)은 관례상 그대로 남아 있지만 값은 라이트다:
  배경 oklch(0.985 0.003 90) (관리 창·설정 창) · oklch(0.99 0.002 90) (트레이)
  테두리 oklch(0.45 0.01 80 / 0.2)
  텍스트 oklch(0.27 0.012 60) / oklch(0.36 0.012 70) / oklch(0.6 0.012 70)
```
...
줄에서 가져왔는지)는 `src/styles/tokens.css`의 각 블록 주석 참조. 설정 창은 `tauri.conf.json`도
560×700으로 맞췄다(2단계 B트랙). 관리 창(보드)은 목업 1040×620과 실제 1040×640이 아직
어긋나 있다 — 사소한 차이라 이번 개편에서는 그대로 두었다.
